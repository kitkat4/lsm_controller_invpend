...
class InvertedPendulum:
    ...
    # torque[Nm]
    # timestep[s]
    def simulate_one_step(self, torque, timestep): 

        tmp1 = torque * 3.0 / (self.m * self.l * self.l)
        tmp2 = 3.0 * self.g / (2.0 * self.l)

        # k*: approximation of theta_dot
        # l*: approximation of theta_ddot
        k1 = self.theta_dot
        l1 = tmp1 + tmp2 * math.sin(self.theta)

        k2 = self.theta_dot + timestep * l1 / 2.0
        l2 = tmp1 + tmp2 * math.sin(self.theta + timestep * k1 / 2.0)

        k3 = self.theta_dot + timestep * l2 / 2.0
        l3 = tmp1 + tmp2 * math.sin(self.theta + timestep * k2 / 2.0)

        k4 = self.theta_dot + timestep * l3
        l4 = tmp1 + tmp2 * math.sin(self.theta + timestep * k3)
